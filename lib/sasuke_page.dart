...
class SasukePage extends StatefulWidget {
  const SasukePage({Key? key}) : super(key: key);

  @override
  _SasukePageState createState() => _SasukePageState();
}

class _SasukePageState extends State<SasukePage> {
  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    futureAlbum = getSasuke();
  }
  @override
          ...
          child: Column(
            ...
            children: [
              ...
              FutureBuilder<Album>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    return Text('${snapshot.data!.description.replaceAll('__', '').replaceAll('~!', '').replaceAll('!~', '')}');
                  } else if (snapshot.hasError) {
                    return Text("${snapshot.error}");
                  }
                  return CircularProgressIndicator();
